...
import { Camera, Square, Play, UserCheck, AlertCircle, Loader2 } from 'lucide-react'
import { BrowserFaceRecognitionService } from '@/lib/face-recognition-browser'
...
export default function FaceRecognitionCameraFixed({ onAttendanceMarked }: FaceRecognitionCameraProps) {
  ...
  const faceRecognitionService = BrowserFaceRecognitionService.getInstance()

  useEffect(() => {
    // Initialize face recognition models on component mount
  ...
  const startFaceRecognition = useCallback(() => {
    ...
    const recognizeInterval = setInterval(async () => {
      ...
      try {
        ...
        // Create canvas from video for processing
        const canvas = faceRecognitionService.createCanvasFromVideo(videoRef.current)
        
        // Resize for better performance
        const resizedCanvas = faceRecognitionService.resizeCanvas(canvas, 320, 240)
        
        // Extract face descriptor
        const faceDescriptor = await faceRecognitionService.extractFaceDescriptor(resizedCanvas)
        ...
        if (faceDescriptor) {
          ...
          if (response.ok) {
            ...
            console.error('Face recognition API error:', response.statusText)
          }
        } else {
          setLastRecognition({
            match: false,
            ...
            <div className="absolute top-4 right-4 bg-black/80 rounded-lg p-3 max-w-xs">
              {lastRecognition.match ? (
                ...
                <div className="flex items-center space-x-2 text-yellow-400">
                  ...
                  <p className="text-sm">{lastRecognition.message || 'No match found'}</p>
                </div>
              )}
            </div>
          )}
